...
def global_rules(world):
    ...
    set_rule(world.get_entrance('Goron Shop'), lambda state: state.has_explosives() or state.has('Progressive Strength') or (state.has_bow() and state.is_adult()) or (state.has('Dins Fire') and state.has('Magic Meter')) or ((state.has('Zeldas Letter') or world.open_kakariko) and (state.can_play('Zeldas Lullaby') and state.has_sticks())))
    ...
    set_rule(world.get_entrance('Front of Meadow Grotto'), lambda state: state.has_explosives() or (state.can_blast_or_smash() and state.is_adult() and (state.can_play('Minuet of Forest') or state.can_play('Sarias Song'))))
    ...
    set_rule(world.get_location('GS Lost Woods Bean Patch Near Stage'), lambda state: state.has_bottle() and (state.can_child_attack() or (not world.shuffle_scrubs and state.has('Buy Deku Shield'))))
...
def dung_rules_dc0(world):
    ...
    # GS
    set_rule(world.get_location('GS Dodongo\'s Cavern East Side Room'), lambda state: state.has_explosives() or state.is_adult() or state.has_slingshot() or state.has('Boomerang') or state.has_sticks() or state.has_explosives() or state.has('Kokiri Sword'))
